...
namespace ScriptableDictionaries.Editor
{
    ...
    public class ScriptableDictionaryEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (length > 0)
            {
                typeName = _properties.values.GetArrayElementAtIndex(0).type.ToString();
            }
            for (int i = 0; i < length; i++)
            {
                DrawKeyValueLine(i,
                    _properties.keys.GetArrayElementAtIndex(i),
                    _properties.values.GetArrayElementAtIndex(i), typeName, Conflict.None);
        ...
        private void DrawKeyValueLine(int index, SerializedProperty keyProp, SerializedProperty valProp, string typeName, Conflict conflict)
        {
            using (new HorizontalScope())
            {
                ...
                PropertyField(valProp, new GUIContent(typeName));
